...
export async function main(ns: NS) {
  ...
  while (true) {
    ...
    // Notify client
    if (totalSpent > 0) {
      ns.toast(`Hacknet: Spent ${money.format(totalSpent)}`);
    }
    ...
    if (hacknetWait > flags.breakeven) {
      ns.toast(`Hacknet: Breakeven point reached (${integer.format(flags.breakeven)}s)`, "warning", 5000);
      break;
    }

    // Break if the RAM on the host is low.
    if (homeRam < flags.ram) {
        ns.toast(`Hacknet: Not enough RAM (${integer.format(homeRam)}/${integer.format(flags.ram)}GB)`, "warning", 5000);
        break;
    }
